...
if [ -f "$(gettop)/bootable/recovery/orangefox.cpp" ]; then
	echo -e "\x1b[96m[INFO]: Setting up OrangeFox build vars for radiant...\x1b[m"
	if [ "$1" = "$FDEVICE" ] || [  "$FOX_BUILD_DEVICE" = "$FDEVICE" ]; then
		...
		export TARGET_DEVICE_ALT="X695C, x695c"
		...
		export FOX_USE_SPECIFIC_MAGISK_ZIP="$(gettop)/device/redmi/radiant/Magisk/Magisk.zip"
